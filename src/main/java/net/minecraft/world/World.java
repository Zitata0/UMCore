			...
			if (!entity.isDead)
			{
				try
				{
					if(balancer.canUpdateEntity(entity))
					{
						...
						if(elapsed > 20000000)
							FMLLog.fine("Possible lag source Entity %s %sms", entity, (elapsed/1000000));
					}
				}
				catch (Throwable throwable1)
				{
...
//				this.loadedEntityList.remove(i--);
		...
		if (!this.field_147483_b.isEmpty())
		{
			for (Object tile : field_147483_b)
			{
				...
				long elapsed = System.nanoTime() - startT;
				if(elapsed > 20000000)
					FMLLog.fine("Possible lag source on unload TileEntity %s [%s](%s, %s, %s) %sms", tile.getClass(), provider.dimensionId,
		...
		while (iterator.hasNext())
		{
			...
			if (!tileentity.isInvalid() && tileentity.hasWorldObj() && activeChunks.containsKey(key))
			{
				...
				try
				{
					...
					long elapsed = System.nanoTime() - startT;
					if(elapsed > 20000000)
						FMLLog.fine("Possible lag source TileEntity %s [%s](%s, %s, %s) %sms", tileentity.getClass(), provider.dimensionId,
